...
class FP8Linear(nn.Module):
    """Custom Linear layer to handle BFL's FP8 weights and scaling factors."""
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        # Initialize as empty float8_e4m3fn, load_state_dict will fill it
        ...
        # Buffers to catch the scales from the safetensors file
        self.register_buffer("weight_scale", torch.ones(()))
        self.register_buffer("input_scale", torch.ones(()))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # Upcast the 8-bit weight to match the input (bfloat16)
        weight_bf16 = self.weight.to(x.dtype)

        # Apply the scaling factor
        scaled_weight = weight_bf16 * self.weight_scale.to(x.dtype)

        # Perform the standard linear projection
...
class SelfAttention(nn.Module):
    def __init__(
        ...
        self.qkv = FP8Linear(dim, dim * 3, bias=False)
...
class SingleStreamBlock(nn.Module):
    def __init__(
        ...
        # Change from nn.Linear to FP8Linear
        self.linear1 = FP8Linear(
            hidden_size,
            hidden_size * 3 + self.mlp_hidden_dim * self.mlp_mult_factor,
            bias=False,
        )
        self.linear2 = FP8Linear(
...
class DoubleStreamBlock(nn.Module):
    def __init__(
        ...
        self.img_mlp = nn.Sequential(
            FP8Linear(
                hidden_size, mlp_hidden_dim * self.mlp_mult_factor, bias=False
            ),  # Changed
            SiLUActivation(),
            FP8Linear(mlp_hidden_dim, hidden_size, bias=False),  # Changed
        )
